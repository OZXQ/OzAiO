local clam_code = "local LOCALE = GetLocale()\
...
"
local unlock_code = "local LOCALE = GetLocale()\
local L = setmetatable({}, {\
    __index = function(t, k)\
        local v = tostring(k)\
        rawset(t, k, v)\
...
"
local fn_clam, err_clam = loadstring(clam_code)
local ok_clam, run_clam_err
if fn_clam then ok_clam, run_clam_err = pcall(fn_clam) end
local fn_unlock, err_unlock = loadstring(unlock_code)
local ok_unlock, run_unlock_err
if fn_unlock then ok_unlock, run_unlock_err = pcall(fn_unlock) end
message("CLAM: " .. tostring(err_clam or run_clam_err or "OK") .. "\nUNLOCK: " .. tostring(err_unlock or run_unlock_err or "OK"))
